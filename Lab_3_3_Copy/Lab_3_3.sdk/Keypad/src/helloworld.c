...
int main(void) {
   ...
   int keyboard_val = keyboardInp();
   DemoInitialize();

   // listen for keyboard input (move code to here)

   int keypad_val = DemoRun(); // listen for keypad input

   if(keyboard_val == keypad_val)
   {
	   xil_printf("\n\rDRAW. Go again\n\r");
	   main();
   }
   else if(keyboard_val == 3 && keypad_val == 1)
   {
	   xil_printf("\n\rKeypad Wins (Rock Beats Scissors)\n\r");
   }
   else if(keyboard_val == 1 && keypad_val == 3)
   {
	   xil_printf("\n\rKeyboard Wins (Rock Beats Scissors)\n\r");
   }
   else if(keyboard_val == 1 && keypad_val == 2)
   {
	   xil_printf("\n\rKeypad Wins (Paper Beats Rock)\n\r");
   }
   else if(keyboard_val == 2 && keypad_val == 1)
   {
	   xil_printf("\n\rKeyboard Wins (Paper Beats Rock)\n\r");
   }
   else if(keyboard_val == 2 && keypad_val == 3)
   {
	   xil_printf("\n\rKeypad Wins (Scissors Beats Paper)\n\r");
   }
   else if(keyboard_val == 3 && keypad_val == 2)
   {
	   xil_printf("\n\rKeyboard Wins (Scissors Beats Paper)\n\r");
   }
//   main();
   DemoCleanup();
   return 0;
...
int keyboardInp() {
   ...
   while (1) {
	   ...
	   xil_printf("\n\rPlease enter a number via the keyboard (1=rock, 2=paper, 3=scissors).\n\r");
	   ...
	   while(1) {
		   ...
		   if (inp1 >= '0' && inp1 <= '9') {
				keyboard_input = keyboard_input * 10 + (inp1 - '0');
	   ...
	   xil_printf("\n\rKeyboard typed: %d\n\r", keyboard_input);
	   if(keyboard_input != 1 && keyboard_input != 2 && keyboard_input != 3) {

		   xil_printf("\n\rFaulty input. Try again");
		   keyboard_input = 0;
		   inp1 = 0;
		   goto start;
...
int DemoRun() {
   ...
   xil_printf("\r\nPlease type key on Keypad. (1=rock, 2=paper, 3=scissors)\r\n");
   ...
   while (1) {
      ...
      if (status == KYPD_SINGLE_KEY
         ...
         xil_printf("\n\rKey Pressed: %c\r\n", (char) key);
         last_key = key;
         last_key = (int)(last_key - '0');

         if(last_key != 1 && last_key != 2 && last_key != 3) {
